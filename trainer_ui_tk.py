# -*- coding: utf-8 -*-
"""
平野孤鸿修改器 - tkinter 微信三栏布局版 v0.4.2
集成完整业务逻辑
...
from src.constants import APP_VERSION, DEFAULT_GAME_PATH, SIM_COMMON_REL
...
class TrainerApp:
    def __init__(self, root):
        ...
        w = win_cfg.get("width") or 1200
        h = win_cfg.get("height") or 750
        x = win_cfg.get("x")
        y = win_cfg.get("y")
        geo = f"{w}x{h}"
        if x and y:
        ...
        self.current_nav = self.config.get("last_nav", 0)
        self.current_func = 0
        self.nav_buttons = []
        self.func_items = []
        self.game_pid = None
        ...
        try:
            ...
            if m:
                if "window" not in self.config:
                    self.config["window"] = {}
                self.config["window"]["width"] = int(m.group(1))
                self.config["window"]["height"] = int(m.group(2))
                if m.group(3) and m.group(4):
                    self.config["window"]["x"] = int(m.group(3))
                    self.config["window"]["y"] = int(m.group(4))
            # 保存当前导航
            self.config["last_nav"] = self.current_nav
    ...
    def _update_status(self):
        try:
            ...
            self.game_pid = result[0] if (result and isinstance(result, tuple)) else None
    ...
    def _build_func_panel(self, parent):
        ...
        self.func_data = {
            ...
            2: [("","快照回滚","保存/回滚游戏状态",False),("","预设方案","保存/加载修改方案",False),("","资源修改","资源+100万/归零",False),("","时间控制","加速/季节/跳天",False),("","天气控制","固定天气",False),
            ...
            5: [("","热键设置","全局热键",False),("","诊断模式","一键排查问题",False),
                ("","日志管理","日志路径",False),("","MOD管理","散文件MOD",False),
                ("","新手引导","使用教程",False),("","关于","版本信息",False)],
        }
    ...
    def _build_tools(self, func):
        tool_map = {
            -2: self._build_snapshots,
            -1: self._build_presets,
    ...
    def _build_snapshots(self):
        g = self._group("📸 游戏状态快照")
        bf = tk.Frame(g, bg=COLORS["bg_card"])
        bf.pack(fill=tk.X, padx=16, pady=12)
        self._btn(bf, "📸 保存当前快照", "primary", self._save_snapshot).pack(side=tk.LEFT, padx=5)
        self._btn(bf, "↩️ 回滚到快照", "danger", self._rollback_snapshot).pack(side=tk.LEFT, padx=5)
        ...
        if hasattr(self, "game_snapshot") and self.game_snapshot:
            tk.Label(bf2, text=f"✅ 已有快照", font=(FONT, 11, "bold"),
                    bg=COLORS["bg_card"], fg=COLORS["success"]).pack(anchor=tk.W, padx=5, pady=2)
            for k, v in self.game_snapshot.items():
                tk.Label(bf2, text=f"  {k}: {v}", font=(FONT, 10),
                        bg=COLORS["bg_card"], fg=COLORS["fg"]).pack(anchor=tk.W, padx=15)
        else:
            tk.Label(bf2, text="❌ 暂无快照", font=(FONT, 11, "bold"),
                    bg=COLORS["bg_card"], fg=COLORS["fg_muted"]).pack(anchor=tk.W, padx=5, pady=2)

    def _save_snapshot(self):
        """保存当前游戏状态快照"""
        try:
            from src.game_status import get_game_status
            status = get_game_status()
            if status:
                self.game_snapshot = {
                    "金钱": status.get("money", "未知"),
                    "人口": status.get("population", "未知"),
                    "时间": status.get("time", "未知"),
                }
                self._add_history("保存游戏状态快照", None)
                log_success("游戏状态快照已保存")
            else:
                log_warning("无法获取游戏状态，请先注入DLL")
        except Exception as e:
            log_error(f"保存快照失败: {e}")
        self._switch_func(self.current_nav, self.current_func)

    def _rollback_snapshot(self):
        """回滚到快照"""
        if not hasattr(self, "game_snapshot") or not self.game_snapshot:
            log_warning("暂无快照可回滚")
            return
        
        # 提示
        if not messagebox.askyesno("确认回滚", "确定要回滚到快照状态吗？"):
            return
        
        try:
            # 这里只是恢复记录，实际回滚需要游戏支持
            # 我们记录当前状态，然后提示用户
            log_success(f"已回滚到快照: 金钱={self.game_snapshot.get('金钱')}")
            self._add_history("回滚到游戏快照", None)
        except Exception as e:
            log_error(f"回滚失败: {e}")
        self._switch_func(self.current_nav, self.current_func)

    def _build_presets(self):
        g = self._group("📋 修改预设方案")
        bf = tk.Frame(g, bg=COLORS["bg_card"])
        bf.pack(fill=tk.X, padx=16, pady=12)
        ...
        if not presets:
            tk.Label(bf, text="暂无预设方案，先保存一个吧", font=(FONT, 10), 
                    bg=COLORS["bg_card"], fg=COLORS["fg_muted"]).pack(pady=10)
        else:
            for name, actions in presets.items():
                row = tk.Frame(bf, bg=COLORS["bg_card"])
                row.pack(fill=tk.X, pady=2)
                tk.Label(row, text=name, font=(FONT, 10), bg=COLORS["bg_card"], 
                        fg=COLORS["fg"]).pack(side=tk.LEFT, padx=5)
                self._btn(row, "应用", "normal", lambda n=name: self._apply_preset(n)).pack(side=tk.LEFT, padx=2)
                self._btn(row, "删除", "normal", lambda n=name: self._delete_preset(n)).pack(side=tk.LEFT, padx=2)
        
        # 保存当前方案
        g2 = self._group("💾 保存当前方案")
        bf2 = tk.Frame(g2, bg=COLORS["bg_card"])
        bf2.pack(fill=tk.X, padx=16, pady=12)
        self.preset_name_entry = tk.Entry(bf2, font=(FONT, 10), width=20)
        self.preset_name_entry.pack(side=tk.LEFT, padx=5)
        self._btn(bf2, "保存", "normal", self._save_preset).pack(side=tk.LEFT, padx=5)
    ...
    def _save_preset(self):
        ...
        # 保存当前创造模式选项
        actions = {}
        for k, var in self.creative_vars.items():
            actions[k] = var.get()
        # 保存当前资源值（如果有）
        if "presets" not in self.config:
            self.config["presets"] = {}
        self.config["presets"][name] = actions
        save_config(self.config)
        log_success(f"预设方案已保存: {name}")
        self._switch_func(self.current_nav, self.current_func)

    def _apply_preset(self, name):
        presets = self.config.get("presets", {})
        if name not in presets:
            log_warning(f"预设不存在: {name}")
            return
        actions = presets[name]
        # 应用到UI
        for k, v in actions.items():
            if k in self.creative_vars:
                self.creative_vars[k].set(v)
        log_success(f"已应用预设方案: {name}")
        # 应用创造模式
        if any(actions.values()):
            self._run_async(self._apply_creative_from_vars)

    def _delete_preset(self, name):
        if "presets" in self.config and name in self.config["presets"]:
            del self.config["presets"][name]
            save_config(self.config)
            log_success(f"已删除预设: {name}")
            self._switch_func(self.current_nav, self.current_func)
    ...
    def _backup_save(self):
        ...
        if not os.path.exists(save_dir):
            log_error("存档目录不存在")
            return
        from src.constants import runtime_config_dir; backup_dir = os.path.join(runtime_config_dir(), "backups")
    ...
    def _build_monitor(self):
        ...
        my_gen = self._monitor_gen
        self._refresh_monitor(my_gen)

    def _refresh_monitor(self, my_gen=None):
        if not hasattr(self, '_monitor_running') or not self._monitor_running:
            return
        if my_gen is not None and my_gen != getattr(self, '_monitor_gen', 0):
            return
        try:
            import psutil
            # === 进程级监控 ===
            ...
            pass
        self.root.after(1500, lambda: self._refresh_monitor(my_gen))
...
return err
    ...
    def _build_hotkey_settings(self):
        ...
        self._btn(bf, "🔍 检测冲突", "normal", self._check_hotkey_conflicts).pack(side=tk.LEFT, padx=5)
        ...
        tk.Label(header, text="操作", font=(FONT, 11, "bold"), bg=COLORS["bg_sidebar"],
                fg=COLORS["fg"], width=10, anchor="w").pack(side=tk.LEFT, padx=8, pady=6)
        ...
        for key, info in HOTKEY_DEFS.items():
            ...
            lbl = tk.Label(row, text=hotkey, font=(FONT, 11, "bold"), bg=COLORS["bg_card"],
                    fg=COLORS["accent"], width=15, anchor="w", cursor="hand2")
            lbl.pack(side=tk.LEFT, padx=8, pady=4)
            lbl.bind("<Double-Button-1>", lambda e, k=key: self._edit_hotkey(k))
            self.hotkey_labels[key] = lbl
            self._btn(row, "修改", "normal", lambda k=key: self._edit_hotkey(k)).pack(side=tk.LEFT, padx=4)

        tk.Label(g, text="提示：双击热键或点修改按钮可自定义热键", font=(FONT, 10),
                bg=COLORS["bg_card"], fg=COLORS["fg_muted"], anchor="w").pack(fill=tk.X, anchor=tk.W, padx=16, pady=8)

    def _edit_hotkey(self, key):
        """弹出热键编辑窗口"""
        win = tk.Toplevel(self.root)
        win.title(f"修改热键 - {HOTKEY_DEFS[key]['name']}")
        win.geometry("300x150")
        win.transient(self.root)
        win.grab_set()
        
        tk.Label(win, text="请按下新的热键组合...", font=(FONT, 12)).pack(pady=20)
        lbl = tk.Label(win, text="", font=(FONT, 14, "bold"), fg=COLORS["accent"])
        lbl.pack(pady=10)
        
        def on_key(e):
            # 解析按键
            keys = []
            if e.state & 0x0004: keys.append("Ctrl")
            if e.state & 0x0001: keys.append("Shift")
            if e.state & 0x0008: keys.append("Alt")
            key_name = e.keysym
            ...
            # 检查冲突
            current_hotkeys = self.config.get("hotkeys", {})
            conflict = None
            for k, v in current_hotkeys.items():
                if k != key and v == hotkey:
                    conflict = k
                    break
            
            lbl.config(text=hotkey)
            if conflict:
                tk.Label(win, text=f"⚠️ 与 {HOTKEY_DEFS[conflict]['name']} 冲突", 
                        fg=COLORS["error"]).pack()
            
            # 确认按钮
            def save():
                if "hotkeys" not in self.config:
                    self.config["hotkeys"] = {}
                self.config["hotkeys"][key] = hotkey
                save_config(self.config)
                log_success(f"热键已更新: {HOTKEY_DEFS[key]['name']} = {hotkey}")
                if key in self.hotkey_labels:
                    self.hotkey_labels[key].config(text=hotkey)
                win.destroy()
            
            btn_frame = tk.Frame(win)
            btn_frame.pack(pady=10)
            tk.Button(btn_frame, text="保存", command=save, bg=COLORS["accent"], 
    ...
    def _check_hotkey_conflicts(self):
        ...
        seen = {}
        conflicts = []
        for key, hotkey in current_hotkeys.items():
            if hotkey in seen:
                conflicts.append((seen[hotkey], key, hotkey))
            else:
                seen[hotkey] = key
        
        if conflicts:
            msg = "发现冲突:\n"
            for k1, k2, hk in conflicts:
                msg += f"  {HOTKEY_DEFS[k1]['name']} 与 {HOTKEY_DEFS[k2]['name']} 都是 {hk}\n"
            log_warning(msg)
            messagebox.showwarning("热键冲突", msg)
        else:
            log_success("未发现热键冲突")
            messagebox.showinfo("热键检测", "✅ 未发现热键冲突")
    ...
    def _build_diagnostic(self):
        ...
        bf = tk.Frame(g, bg=COLORS["bg_card"])
        bf.pack(fill=tk.X, padx=16, pady=12)
        self._btn(bf, "🚀 开始诊断", "primary", self._run_diagnostic).pack(side=tk.LEFT, padx=5)
        
        # 诊断结果
        self.diag_text = tk.Text(g, font=(FONT, 10), bg=COLORS["bg_card"], 
                                fg=COLORS["fg"], height=15, wrap=tk.WORD)
        self.diag_text.pack(fill=tk.BOTH, expand=True, padx=16, pady=8)
    ...
    def _run_diagnostic(self):
        ...
        if os.path.exists(game_path):
            add(f"   ✅ 游戏路径存在: {game_path}")
        else:
            add(f"   ❌ 游戏路径不存在: {game_path}")
        add("")
        ...
        if os.path.exists(dll_path):
            ...
            add(f"   ✅ DLL存在: {dll_path} ({size//1024}KB)")
        else:
            add(f"   ❌ DLL不存在: {dll_path}")
        add("")
        ...
        if os.path.exists(CONFIG_PATH):
            add(f"   ✅ 配置文件存在: {CONFIG_PATH}")
        else:
            add(f"   ⚠️ 配置文件不存在（首次运行正常）")
        add("")
        ...
        self.diag_text.config(state=tk.DISABLED)
        log_success("诊断完成，请查看结果")

    def _build_guide(self):
        g = self._group("📖 新手引导")
        bf = tk.Frame(g, bg=COLORS["bg_card"])
        bf.pack(fill=tk.BOTH, expand=True, padx=16, pady=12)
...
A: 关闭创造模式，不要同时开太多修改
"""
        text = tk.Text(bf, font=(FONT, 10), bg=COLORS["bg_card"], 
                      fg=COLORS["fg"], wrap=tk.WORD)
    ...
    def _build_log_settings(self):
        ...
        # 操作历史
        g2 = self._group("📜 操作历史（最近20条）")
        self.history_frame = tk.Frame(g2, bg=COLORS["bg_card"])
        self.history_frame.pack(fill=tk.BOTH, expand=True, padx=16, pady=8)
        
        if not hasattr(self, "action_history"):
            self.action_history = []
        
        if not self.action_history:
            tk.Label(self.history_frame, text="暂无操作记录", font=(FONT, 10),
                    bg=COLORS["bg_card"], fg=COLORS["fg_muted"]).pack(pady=10)
        else:
            for i, (time, action, undo_fn) in enumerate(reversed(self.action_history[-20:])):
                row = tk.Frame(self.history_frame, bg=COLORS["bg_card"])
                row.pack(fill=tk.X, pady=1)
                tk.Label(row, text=f"{time}", font=(FONT, 9), bg=COLORS["bg_card"],
                        fg=COLORS["fg_muted"], width=12, anchor="w").pack(side=tk.LEFT, padx=3)
                tk.Label(row, text=action, font=(FONT, 10), bg=COLORS["bg_card"],
                        fg=COLORS["fg"], anchor="w").pack(side=tk.LEFT, padx=3, fill=tk.X, expand=True)
                if undo_fn:
                    self._btn(row, "撤销", "normal", 
                             lambda fn=undo_fn: self._undo_action(fn)).pack(side=tk.LEFT, padx=3)

    def _add_history(self, action, undo_fn=None):
        """添加操作历史"""
        from datetime import datetime
        time_str = datetime.now().strftime("%H:%M:%S")
        if not hasattr(self, "action_history"):
            self.action_history = []
        self.action_history.append((time_str, action, undo_fn))
        # 只保留最近50条
        if len(self.action_history) > 50:
            self.action_history = self.action_history[-50:]

    def _undo_action(self, undo_fn):
        """执行撤销操作"""
        try:
            undo_fn()
            log_success("已撤销上一步操作")
        except Exception as e:
            log_error(f"撤销失败: {e}")
        self._switch_func(self.current_nav, self.current_func)
