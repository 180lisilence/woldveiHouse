...
/* 特征码扫描：lua_pcall 常见开头指令模式
 * 不同版本Lua可能有差异，这里用通用模式：
 * push rbp; mov rbp, rsp; sub rsp, XX 或类似 prolog
 */
static void* scan_lua_pcall(HMODULE mod) {
    if (!mod) return NULL;
    
    /* 获取模块信息 */
    MODULEINFO mod_info;
    if (!GetModuleInformation(GetCurrentProcess(), mod, &mod_info, sizeof(mod_info))) {
        log_msg("[Scan] GetModuleInformation 失败");
        return NULL;
    }
    
    BYTE *base = (BYTE*)mod_info.lpBaseOfDll;
    DWORD size = mod_info.SizeOfImage;
    
    log_msg("[Scan] 扫描 Lua5X64.dll: %p, 大小: %d", base, size);
    
    /* lua_pcall 常见开头：55 48 8B EC 48 83 EC ?? (push rbp; mov rbp,rsp; sub rsp,XX) */
    BYTE pattern[] = { 0x55, 0x48, 0x8B, 0xEC, 0x48, 0x83, 0xEC };
    ...
    for (DWORD i = 0; i < size - pattern_len; i++) {
        ...
        if (match) {
            /* 验证：后面应该有 call 或 jmp 到 luaD_pcall */
            log_msg("[Scan] 找到疑似 lua_pcall: %p", p);
            return p;
        }
    }
    
    log_msg("[Scan] 未找到 lua_pcall 特征码");
    return NULL;
}
...
static void* find_lua_pcall() {
    ...
    if (addr) {
        log_msg("[Hook] lua_pcall 地址（导出）: %p", addr);
        return addr;
    }
    
    log_msg("[Hook] 未找到 lua_pcall 导出，尝试特征码扫描...");
    
    /* 2. 导出找不到，用特征码扫描 */
    addr = scan_lua_pcall(g_lua_dll);
    if (addr) {
        log_msg("[Hook] lua_pcall 地址（扫描）: %p", addr);
        return addr;
    }
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved) {
    switch (fdwReason) {
        case DLL_PROCESS_ATTACH: {
            ...
            /* 初始化通信路径：DLL所在目录/dist */
            strncpy(g_comm_dir, g_log_file, MAX_PATH - 1);
            /* g_log_file 此时末尾是 \，追加 dist */
            strcat(g_comm_dir, "dist");
            CreateDirectoryA(g_comm_dir, NULL);
            snprintf(g_cmd_file, MAX_PATH, "%s\\lua_cmd.txt", g_comm_dir);
            snprintf(g_result_file, MAX_PATH, "%s\\lua_result.txt", g_comm_dir);
