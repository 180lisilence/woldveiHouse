...
>
> 纯内存操作 · DLL 注入 · Lua 执行引擎 · 微信三栏布局 · 12 个全局热键 · 深/浅主题 GUI
...
1. **下载**：获取 `woldvein_trainer_v0.4.2.exe` 和 `woldvein_trainer.dll`，放在同一目录
2. **启动游戏**：通过 Steam 启动《平野孤鸿》
3. **启动修改器**：右键 `woldvein_trainer_v0.4.2.exe` → 以管理员身份运行（热键需要）
...
│   └── woldvein_trainer_v0.4.2.exe    # 打包版主程序（PyInstaller onedir）
│
...
pyinstaller --onedir --windowed --name "woldvein_trainer_v0.4.2" `
...
```
dist/
└── woldvein_trainer_v0.4.2/            # onedir 输出目录
    ├── woldvein_trainer_v0.4.2.exe     # 主程序
...
*README 版本：v0.4.2 详细版 | 更新日期：2026-09-14*
